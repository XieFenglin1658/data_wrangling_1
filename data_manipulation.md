...
\##tidyverse中function的输入套路就是：要处理的数据+相应动作,且输入data
frame,输出一定是data frame.
...
\##keep someting \##select specific thing
...
用pull function以及下面那个code的结果输出不是data
frame,只是vector,不要用这个function。select输出的是data
frame,就算你只选了一个variable!
...
\##如果我想去save处理过后的data frame,just重新命名它：“litters_df =
select(litters_df, GROUP = group, everything())”
...
using `filter` to filter out rows. \##“==”–R来判断true or false
...
\##若什么都不加，如第一个code，则drop_na会把所有missing
value全部清理出去，若加上处理对象（第二条code)则只会处理该对象的missing
value.
...
use `mutate` to create or modify variables str_to_lower is a function to
lower the case of string variable
